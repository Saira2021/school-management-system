            ...
            <?php if($type == '1'){ ?>
                <form method="post" class="signin-form">
                            ...
                            <select name="subject_id" id="subject_id" class="form-control" required>
                                ...
                                if(mysqli_num_rows($get_subject)){
                                    while($q = mysqli_fetch_assoc($get_subject)){
                                        ...
                                        $room_id = $q['room_id'];
                                        $get_room = mysqli_query($db, "SELECT * FROM class_room WHERE id = '$room_id' ");
                                        if(mysqli_num_rows($get_room)){
                                            $room_data = mysqli_fetch_assoc($get_room);
                                            $room_name = $room_data['name'];
                                                $class_id = $room_data['class_id'];
                                                $get_class_a = mysqli_query($db, "SELECT * FROM class WHERE id = '$class_id' ");
                                        ...
                                        ?>
                                        <option value="<?php echo $q['id'] ?>"><?php echo $class_name ?> | <?php echo $room_name ?> | <?php echo $q['name'] ?></option>
                                        <?php
                                    }
                                }
                    ...
                    if(isset($_POST['generate_btn'])){
                        ...
                        $check = mysqli_query($db, "SELECT * FROM timetable WHERE subject_id = '$subject_id' AND day = '$day' AND time = '$time' ");
                        if(mysqli_num_rows($check) != 0){
                            echo "<br><p class='alert alert-warning'>Already Exist.</p>";
                        }else{
                            $insert = mysqli_query($db, "INSERT INTO timetable(subject_id, `day`, `status`, `time`) VALUES('$subject_id','$day','0', '$time') ");
                            if($insert){
...
<!-- // ROOM Search -->
                    ...
                    if(isset($_POST['subject_search'])){
                        ...
                        $select = mysqli_query($db, "SELECT * FROM timetable WHERE subject_id = '$subject_id' AND status = 1 ");
                                ...
                                if(mysqli_num_rows($subject_get)){
                                    ...
                                    $subject_room_id = $subject_data['room_id'];
                                        ...
                                        if(mysqli_num_rows($faculty)){
                                            ...
                                            $room = mysqli_query($db, "SELECT * FROM class_room WHERE id = '$subject_room_id' AND class_id = '$class_id' ");
                                            if(mysqli_num_rows($room)){
                                                $room_data = mysqli_fetch_assoc($room);
                                                $room_name = $room_data['name'];
                                                $room_class_id = $room_data['class_id'];
                                ...
                                if(mysqli_num_rows($subject_get)){
                                    ...
                                    $subject_room_id = $subject_data['room_id'];
                                        ...
                                        if(mysqli_num_rows($faculty)){
                                            ...
                                            $room = mysqli_query($db, "SELECT * FROM class_room WHERE id = '$subject_room_id' AND id = '$room_id' ");
                                            if(mysqli_num_rows($room)){
                                                $room_data = mysqli_fetch_assoc($room);
                                                $room_name = $room_data['name'];
                                                $room_class_id = $room_data['class_id'];
